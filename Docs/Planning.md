...
- Route guards (functional `authGuard`, protects dashboard/applications/analytics/profile)
...
- `core/models`: `Application` + `ActivityEvent` TS interfaces
- Dashboard, Applications, Analytics, Profile: flesh out from empty shells into real UI backed by a mock `ApplicationsService` (swappable for real HTTP in Phase 3)
...
- Resume uploads via S3 (pre-signed URL endpoint), `resume_key` stored on Application
